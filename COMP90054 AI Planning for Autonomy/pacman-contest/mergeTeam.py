...
class AttackAgent(CaptureAgent):
    ...
    def aStarSearch(self, problem, gameState, heuristic):
        ...
        # init
        def get_node(suc,parent_node, problem = problem,heuristic = heuristic):
            node = {}
            node['s'] = suc[0]
            node['p'] = parent_node
            if parent_node == None:
            ...
            node['c'] = c + suc[2]
            node['a'] = suc[1]
            node['f'] = heuristic(node['s']) + node['c']
            return node
        def reconstruct_action(node):
            action = []
            while node['p'] != None:
                action.append(node['a'])
                node = node['p']
            action.reverse()
            return action
        frontier = util.PriorityQueue()
        state = problem.getStartState(gameState,self.foodGrid)
        start = (state,None,0)
        node = get_node(start,None)
        ...
        while not frontier.isEmpty():
            ...
            if elapsed >= 0.8:
                 if debug:
                     print("MergeTeam Time used:", elapsed)
                     print("time exceed")
                 return "TIMEEXCEED",None
            else:
                current = frontier.pop()
                if problem.isGoalState(gameState,current['s']):
                    print("MergeTeam Time Used:",elapsed)
                    return reconstruct_action(current),current['s'][0]
                closed.append(current['s'])
                for successor in problem.getSuccessors(current['s']):
                    state = successor[0]
                    if state in closed:
                        continue
                    else:
                        node = get_node(successor,current)
                        frontier.push(node,node['f'])
        print("MergeTeam Time Used,Not Found:",elapsed)
        return None,None
        #
        # visited = set()
        # best_g = dict()
